function generateDiagnosisData(patientDiagnosisData) {
	...
	for(diagnosis=0;diagnosis<patientDiagnosisData.length;diagnosis++){
		...
		logger.debug("patientDiagnosisData[enc]['code'] ->>> "+patientDiagnosisData[diagnosis]['code'])
			...
			entry_class.appendChild(<effectiveTime><low value={patientDiagnosisData[diagnosis]['date']}/><high nullFlavor='UNK'/></effectiveTime>)
			...
			observation_class.appendChild(<effectiveTime><low value={patientDiagnosisData[diagnosis]['date']}/><high nullFlavor='UNK'/></effectiveTime>)
